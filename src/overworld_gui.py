...
from .heroes import HEROES_BY_FACTION, get_heroes_for_faction
...
class OverworldGUI:
    ...
    def _is_hidden_objective_guard(self, army, my_faction):
        return is_hidden_objective_guard(army, my_faction, self.world.get_objective_at)
    ...
    def _pick_target_army(armies, my_player):
        for a in armies:
            if a.player not in (NEUTRAL_PLAYER, my_player):
                return a
        for a in armies:
    ...
    def _show_objective_reward_dialog(self, faction_name, upgrade_ids, on_select):
        ...
        for upgrade_id in upgrade_ids:
            ...
            summaries = upgrade_effect_summaries(upgrade, ALL_UNIT_STATS, faction_units)
            keywords = upgrade_effect_keywords(upgrade, ALL_UNIT_STATS, faction_units)
            if summaries:
    ...
    def _draw(self):
        ...
        if show_reachable:
            ...
            for a in w.armies:
                if (
                    a.player != self.selected_army.player
                    and not self._is_hidden_objective_guard(a, my_faction)
                ):
                    neighbors.add(a.pos)
    ...
    def _on_right_click(self, event):
        ...
        # Enemy -> battle
        if is_enemy:
            if clicked_army.player == NEUTRAL_PLAYER:
    ...
    def _start_battle(self, attacker, defender):
        ...
        def on_battle_complete(winner, p1_survivors, p2_survivors):
            ...
            if ow_winner == attacker.player and defender.player == NEUTRAL_PLAYER:
    ...
    def _open_split_dialog(self, dest_pos, dest_armies):
        ...
        if dest_armies:
            for dest_army in dest_armies:
                owner = (
                    "Neutral"
                    if dest_army.player == NEUTRAL_PLAYER
                    else f"P{dest_army.player}"
                )
